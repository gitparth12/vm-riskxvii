...
void lb_h(blob* p_vm, uint32_t reg_index, uint32_t address) {
    ...
    while (current != NULL) {
        if (address >= current->start_address && (address+3) <= (current->start_address + current->size)) {
            address -= current->start_address;
            p_vm->registers[reg_index] = (int8_t) (current->p_data[address] & 0xff);
            return;
        }
        current = current->next;
    }
...
void lbu_h(blob* p_vm, uint32_t reg_index, uint32_t address) {
    ...
    while (current != NULL) {
        if (address >= current->start_address && (address+3) <= (current->start_address + current->size)) {
            address -= current->start_address;
            printf("lbu_h value: %d\n", current->p_data[address]);
            p_vm->registers[reg_index] = (current->p_data[address] & 0xff);
            return;
        }
        current = current->next;
    }
...
void lhu_h(blob* p_vm, uint32_t reg_index, uint32_t address) {
    ...
    while (current != NULL) {
        if (address >= current->start_address && (address+3) <= (current->start_address + current->size)) {
            ...
            printf("lhu_h value: %d\n", combined);
            p_vm->registers[reg_index] = (combined & 0xffff);
            return;
        }
        current = current->next;
    }
